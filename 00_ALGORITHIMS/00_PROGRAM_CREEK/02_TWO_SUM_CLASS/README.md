### two sum using classes

This problem was solved previously, but we are going to change the
code a little better and do the following:

1. `add()` adds a number to an internal data structure using the ``add()`` method
2. `find()` function find here exists a pair of numbers which sum to equal the value
3. `find()` returns True or False, not the indices.
